...
### 配置 AI 助手（DeepSeek API）

AI 助手功能依赖 DeepSeek API，需要自行配置 API 密钥：

1. 访问 https://platform.deepseek.com/ 注册账号
2. 在 API Keys 页面创建 API Key
3. 复制 `backend/ai_config.template.py` 为 `backend/ai_config.py`
4. 将密钥填入 `ai_config.py` 中

```bash
# 在服务器上执行
cp backend/ai_config.template.py backend/ai_config.py
